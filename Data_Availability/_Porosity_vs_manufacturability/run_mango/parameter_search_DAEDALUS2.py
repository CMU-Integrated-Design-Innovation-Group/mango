...
def cell_constraints(input_vars):
    ...
    if bounding_box.a < 25 or bounding_box.a > 120:
        return True
    if bounding_box.b < 25 or bounding_box.b > 120:
        return True
    if bounding_box.c < 25 or bounding_box.c > 120:
        return True
...
def absolute_error(input_vars, extra_params):
    # Consider how much scaffold is used:
    edge_lengths = input_vars['edge_lengths']
    actual_scaffold_estimate = sum((edge_lengths / 0.34).astype('int')) * 2
    ideal = extra_params['ideal_length']
    error = np.abs((actual_scaffold_estimate - ideal) / ideal)
...
def create_box():
    ...
    design_space = PolyhedralSpace(bounding_box=new_box, preserved=preserved_regions, excluded=excluded_regions,
                                   ...
                                   #temp_geometry_path='./temp',
                                   #automated_algorithm_software='/Users/kodak/Desktop/PERDIX-Mac/DAEDALUS2',
                                   #automated_name='DAEDALUS')
                                   )
    ...
    constraints = PolyhedralDefaultConstraints(min_face_angle=20,
                                               max_number_basepairs_in_scaffold=8000,
                                               extra_constraints=[cell_constraint,
                                                                  r_cut_constraint,
                                                                ])

    # Create objective function objects:
    objective_1 = ObjectiveFunction(name='Porosity', objective_equation=porosity_objective, numDecimals=2,
                                    extra_params={'d': 4.25, 'ideal_length': 6000})  # DAEDALUS ~ 4nm diameter bundles

    objective_2 = ObjectiveFunction(name='Absolute Error', objective_equation=absolute_error, numDecimals=2,
                                    extra_params={'ideal_length': 6000})
    ...
    extension_ramp2 = Ramp(unique_id='Extension Ramp Parallelepiped', min_value=1, max_value=5,
                           max_number_epochs=num_epochs, min_steps_at_min=10)
    rotation_ramp = Ramp(unique_id='Rotation Ramp Edge', min_value=1, max_value=10,
                         max_number_epochs=num_epochs, min_steps_at_min=10)
    ...
    cset = ['Outside Design Space',
            'Invalid Edge Length', 'Invalid Face Angle',
             'Intersecting faces']

    savename = 'DAEDALUS_' + str(random_seed)
    optimizer = MOSA(
        ...
        max_time_of_optimization_minutes=1200  # Just setting an upper bound in minute to terminate the process
    )
