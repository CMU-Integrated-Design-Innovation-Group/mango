...
def scaffold_constraint(input_vars, extra_params):
    total_length_nts = int(1.8 * (sum(input_vars['edge_lengths']) / 0.34))
...
def absolute_error(input_vars, extra_params):
    ...
    actual_scaffold_estimate = sum((edge_lengths / 0.34).astype('int')) * 1.6
...
def create_box():
    ...
    # Custom scaf constraint calc for vHelix
    scaf_constraint = CustomDesignConstraint(name='Scaffold Constraint',
                                             design_constraint=scaffold_constraint,
                                             extra_params={'max_scaffold_length': 6300})
    ...
    objective_1 = ObjectiveFunction(name='Porosity', objective_equation=porosity_objective, numDecimals=2,
                                    extra_params={'d': 3.15, 'ideal_length': 6000})
